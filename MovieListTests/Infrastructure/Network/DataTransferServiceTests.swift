...
final class DataTransferServiceTests: XCTestCase {
    ...
    func test_whenReceivedValidJsonInResponse_shouldDecodeResponseToDecodableObject() {
        ...
        let networkService = NetworkService(config: config, sessionManager: NetworkSessionManagerMock(response: nil,
                                                                                                      data: responseData,
                                                                                                      error: nil))
    ...
    func test_whenBadRequestReceived_shouldRethrowNetworkError() {
        ...
        let response = HTTPURLResponse(url: URL(string: "test_url")!,
                                       ...
                                       httpVersion: "1.1",
                                       headerFields: nil)
        let networkService = NetworkService(config: config, sessionManager: NetworkSessionManagerMock(response: response,
                                                                                                      data: responseData,
